...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
    ...
    if(req.method === 'POST') {
        ...
        const {email, hashToken, expires } = req.body;

        if(!email || !hashToken || !expires) {
        ...
        if(!isUser){
            return res.json({status: 401, description: "Usuário não cadastrado."});
        }
        ...
        return res.json({status: 200, description: "Aguardando redefinição de senha."});
